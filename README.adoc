...
To activate this extension, add the following line to [.path]_config.rb_ file for your site:
...
Start by adding the following Ruby code at the location [.path]_lib/permalink.rb_.
...
Now you can customize the destination path of any AsciiDoc file by adding the following attribute entry to the document header:
