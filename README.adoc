...
NOTE: If you specify a time zone in the revdate, that time zone is honored.
...
If you don't specify a time zone in the page's revdate or for the application, dates are assumed to be UTC.
...
If you don't set the layout in either of these places, the default layout is considered unset.
...
If the default layout and the `page-layout` attribute are unset, the AsciiDoc processor will generated a standalone document (`header_footer: true`).
