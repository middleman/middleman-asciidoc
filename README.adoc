...
To mark a page as draft (i.e., not published), assign the value `false` the page attribute named `page-published` in the AsciiDoc document header, as follows:
...
Recall that this extension converts the value of page attributes as a YAML value, which means the string literal "`false`" becomes the boolean value `false`.
Middleman then knows not to publish this page.

If you're using the Middleman Blog extension, another option is to set the date of the article way into the future.
